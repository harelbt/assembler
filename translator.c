...
#include "words.h"
void first_pass_translation(line* sentence, line_indexes* indexes, line_counters* counters){
    if (sentence->flags.is_code == TRUE){
        word first_word;
        first_word.ARE = 0b100;
        first_word.OPCODE = sentence->code_parts.opcode;
        first_word.FUNCTION = sentence->code_parts.function;
